...
{
    char *p=getenv("USER"); // to get the user name who is trying to communicate
    char buf[1024];
    ...
    int i;
    struct timeval tp;
    unsigned long long mytime, othertime; // to compare the time to decide which one to talk first
    ...
    while ((argv[1] != NULL) && (i < 4)) {
       send_msg_type = send_msg_type*128 + toascii(argv[1][0]); // for distict msg types using username of reciever
       argv[1]++; i++;
    }


    //ASCII base 128 : a*(128)^3 + b*(128)^2 + c*(128) + d
    i = 0;
    char *q=p;
    while ((q != NULL) && (i < 4)) {
       recv_msg_type = recv_msg_type*128 + toascii(q[0]); // to get msg type for sender using its username
       q++; i++;
    }
    ...
    //asymmetricity necessary for communication
