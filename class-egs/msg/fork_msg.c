...
#include <sys/msg.h> //for msg calls msg parssing 
#include <sys/time.h>

#define SUM_KEY 19
#define SUM_TYPE 1
#define SIZE 1000000000 // 10^9 // length of vector to compute addition on it
...
{
	int *a, i, queue_id;
        msg_t msg; //msg initialized
	unsigned long long child_sum=0, parent_sum=0;
        struct timeval tp_start, tp_end;

        a = (int*)malloc(SIZE*sizeof(int)); // allocating the vector with size

	for (i=0; i<SIZE; i++) a[i] = i; // initiallizing values in vector

	queue_id = msgget(SUM_KEY, 0777 | IPC_CREAT); // getting msg queue descriptor //-----------------------------------------------------------------------------------
	...
	if (fork() == 0) {
		...
		for (i=0; i<SIZE/2; i++) child_sum += a[i]; // geting child part sum
