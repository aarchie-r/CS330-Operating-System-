...
int *a, doneChild=0, doneParent=0;
...
int add (void *arg)
{
	...
	printf("Child saying: my sum is %llu\n", child_sum);
        doneChild=1;
        while (!doneParent) sleep(0);	
                    // Compiler won't register-allocate doneParent across system calls
                        // because it assumes the worst-case scenario: doneParent may be updated
                        // by the system call handler.

                        // So, doneParent is loaded from memory in each iteration.
                        // Could use any system call in the loop body.
}
...
int main (void)
{
	...
	// wait for child
	while(!doneChild) sleep(0);	
                                        // Compiler won't register-allocate doneChild across system calls
                                        // because it assumes the worst-case scenario: doneChild may be updated
                                        // by the system call handler.

                                        // So, doneChild is loaded from memory in each iteration.
                                        // Could use any system call in the loop body.
