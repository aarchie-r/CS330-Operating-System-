...
pthread_cond_t cv = PTHREAD_COND_INITIALIZER; // initializing condition var--------------
int done = 0;
...
void *solver (void *param)
{
	...
	private_sum[id] = 0;
	   ...
	   for (i=(SIZE/num_threads)*id; i<j; i++) {
        ...
        if (done == num_threads - 1) pthread_cond_signal(&cv); // to wake one of the thread waiting
...
int main (int argc, char *argv[])
{
	...
	for (i=1; i<num_threads; i++) {
	   ...
	   for (i=0; i<j; i++) {
        ...
        //Parent waiting for all children to complete
        pthread_mutex_lock(&lock);
        while (done != num_threads - 1) pthread_cond_wait (&cv, &lock);
        pthread_mutex_unlock(&lock);

        for (i=1; i<num_threads; i++) sum += private_sum[i];
