...
void *producer (void *param)
{
   ...
   for (i=0; i<num_items; i++) {
      ...
      // at this point the producer may get switched out thus tail pointer in increased but the buffer is not yet alloted.
      // this will create an empty slot in between or the old value not changed as after consuming it doesn't change the value
       

      pthread_mutex_lock(&lock);
      while (num_in_buffer == SIZE) pthread_cond_wait(&deleted, &lock); // wait if buffer is full
      buffer[index] = produce(i, tid);
      num_in_buffer++;
      pthread_cond_signal(&inserted); // wake up a consumer waiting for produced item in case of empty buffer
...
void *consumer (void *param)
{
   ...
   for (i=0; i<(num_items*num_prods)/num_cons; i++) {
       ...
       // to get index to consume
      pthread_mutex_lock(&lock_delete);
      index = head;
      head = (head + 1) % SIZE;
      pthread_mutex_unlock(&lock_delete);

      // same for consumer num_items not yet increased so other consumer can re-read another value after reading whole buffer

      pthread_mutex_lock(&lock);
      while (num_in_buffer == 0) pthread_cond_wait(&inserted, &lock);
      v = buffer[index];
      num_in_buffer--;
      pthread_cond_signal(&deleted);  // wake up a producer waiting for empty space in case of full buffer
      pthread_mutex_unlock(&lock);
