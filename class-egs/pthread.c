...
#define SIZE 1000000000 // length of array

int *a, num_threads;
unsigned long long *private_sum;

void *solver (void *param) // must be void* return type & passing type
{
	int i, id = *(int*)(param), j;
	private_sum[id] = 0;

        if (num_threads == 2) { // special case taken since right shift is much cheaper than division
...
int main (int argc, char *argv[])
{
	...
	private_sum = (unsigned long long*)malloc(num_threads*sizeof(unsigned long long));
        ...
        tid = (int*)malloc(num_threads*sizeof(int)); // id of threads
        for (i=0; i<num_threads; i++) tid[i] = i;
         //starting index of array for each thread to compute-> (SIZE/num_threads)*tid
         // end point ->  (SIZE/num_threads)*(tid+1) - 1
